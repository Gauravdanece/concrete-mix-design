...
class ConcreteMixDesign:
    ...
    def set_input_parameters(self, params):
        """Web app के लिए input parameters set करें"""
        self.mix_data.update(params)
    
    def calculate_target_strength(self):
        """Calculate target mean strength as per clause 4.2"""
        grade = self.mix_data['grade']
        f_ck = self.grade_properties[grade]['f_ck']
        
        # Adjust standard deviation for fair site control
        ...
        # Calculate both values and take higher
        f_ck1 = f_ck + 1.65 * S
        f_ck2 = f_ck + X
    ...
    def get_air_content(self):
        """Get approximate air content from Table 3"""
        ...
        return air / 100  # Convert to decimal
    
    def select_water_cement_ratio(self, target_strength):
        """Select water-cement ratio based on target strength and cement type"""
        ...
        # Check against maximum for durability
    ...
    def calculate_water_content(self):
        """Calculate water content as per clause 5.3 and Table 4"""
        max_size = self.mix_data['max_aggregate_size']
        slump = self.mix_data['workability_slump']
        agg_type = self.mix_data.get('aggregate_type', 'Crushed angular aggregate')
        
        # Base water content from Table 4 for 50mm slump - angular aggregate
        base_water = {
            10: 208,
            20: 186, 
            40: 165
    ...
    def calculate_cement_content(self, water_content, wc_ratio):
        """Calculate cement content as per clause 5.4"""
    ...
    def calculate_aggregate_proportions(self, wc_ratio):
        """Calculate coarse and fine aggregate proportions"""
        max_size = self.mix_data['max_aggregate_size']
        zone = self.mix_data['fine_agg_zone']
        placing_method = self.mix_data.get('placing_method', 'Chute (Non pumpable)')
        
        # Base volume of coarse aggregate for w/c = 0.50 from Table 5
        base_volume = {
    ...
    def perform_full_design(self):
        """Perform complete mix design calculation"""
        try:
...
def main():
    ...
    with st.sidebar:
        ...
        # Exposure Conditions with descriptions
        exposure_options = {
        ...
        # Aggregate Parameters in columns
        col1, col2 = st.columns(2)
        with col1:
            max_size = st.selectbox("**Max Aggregate Size**", [10, 20, 40], index=1, help="Nominal maximum size of coarse aggregate")
        with col2:
            zone = st.selectbox("**Fine Aggregate Zone**", ['I', 'II', 'III', 'IV'], index=1, help="Grading zone of fine aggregate")
        
        # Workability
        slump = st.slider("**Slump (mm)**", 25, 150, 75, help="Workability measure - 25-50mm for mass concrete, 75-100mm for RCC")
        ...
        site_control = st.radio("**Site Control Quality**", ['Good', 'Fair'], index=0, 
                               help="Good: Well-controlled site with testing lab | Fair: Normal level of control")
    ...
    with tab3:
        ...
        **Usage:**
        1. Select your design parameters in the sidebar
        2. Click 'Calculate Mix Design' button
        3. Review the results and recommendations
        4. Conduct trial mixes for verification
        
        **Note:** This tool is for preliminary mix design. Always verify with actual material tests and trial mixes.
        """)
        
        st.info("""
        **Disclaimer:** This application is designed for educational and professional use. 
        The results should be verified by qualified civil engineers and through laboratory tests.
        """)
